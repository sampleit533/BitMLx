...
% Left-ragged auto-width column for tabularx (prevents right-margin overflow
% in the narrow IEEEtran columns while avoiding over-stretched justification).
\newcolumntype{L}{>{\RaggedRight\arraybackslash}X}
% Let TeX absorb the last few points of long unbreakable code tokens in body
% text instead of pushing them into the right margin.
\setlength{\emergencystretch}{3em}
% Allow long \url / \path strings to break at most punctuation.
...
\begin{tabular}{c>{\RaggedRight\arraybackslash}p{0.72\columnwidth}}
\toprule
Symbol & Description \\
\midrule
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\columnwidth}{@{}>{\RaggedRight\arraybackslash}p{0.22\columnwidth} L L@{}}
\toprule
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\columnwidth}{@{}>{\RaggedRight\arraybackslash}p{0.50\columnwidth} L@{}}
\toprule
Path & Responsibility \\
\midrule
...
Table~\ref{tab:results} presents the compilation results for all four applications. Throughout the evaluation tables and figures we abbreviate the applications as RCD (ReceiverChosenDenomination), TPA (TwoPartyAgreement), MPE (MultichainPaymentExchange), and MLM (MultichainLoanMediator).

\begin{table}[t]
\caption{Compilation results for the four cross-chain applications. Codes: RCD, TPA, MPE, MLM (see text).}
\label{tab:results}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\toprule
Application & \begin{tabular}[b]{@{}r@{}}Trans.\\per chain\end{tabular} & \multicolumn{2}{c}{Wall-clock time (s)} \\
\midrule
RCD & 51 & 3.086 & 0.156 \\
TPA & 51 & 3.867 & 1.860 \\
MPE & 207 & 6.181 & 1.569 \\
MLM & 4{,}098 & 107.461 & 3.447 \\
\bottomrule
\end{tabular}
\end{table}
...
\draw[->] (0,0) -- (5.6,0);
\node[font=\scriptsize] at (2.6,-0.75) {Transactions per chain (log scale)};
\draw[->] (0,0) -- (0,4.8) node[above, font=\scriptsize] {Wall-clock time (s)};
